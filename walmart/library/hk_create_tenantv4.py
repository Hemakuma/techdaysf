...
def main():
  ...
  protocol = module.params['creds']
  host = socket.gethostbyname(module.params['creds']['host'])
...
#
#   #load the error into a python dictionary
#   response1 = json.loads(response)
#   # use aci gui inspector to get this data
#   #codenum = response1["imdata"][0]["error"]["attributes"]["code"]
#   changed = False
#
# #when a object is creted, it returns a empty list eg imdata[] ..see the inspector
# # to check to empty list , just not list ...imdata has list of stuff [] ..checkout when the object already exists
#   if not response1["imdata"]:
#       if action == 'post':
#         changed = True
#       else:
#         changed = False
#
#   elif response1["imdata"][0]["error"]["attributes"]["code"] == '103':
#       if action == 'post':
#         changed = False
#
#   else:
...
#  module.exit_json(changed=False, meta=codenum)
